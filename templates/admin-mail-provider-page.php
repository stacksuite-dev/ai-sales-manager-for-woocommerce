...
$aisales_provider = isset( $aisales_settings['provider'] ) ? $aisales_settings['provider'] : 'default';
$aisales_smtp     = isset( $aisales_settings['smtp'] ) && is_array( $aisales_settings['smtp'] ) ? $aisales_settings['smtp'] : array();
$aisales_sendgrid = isset( $aisales_settings['sendgrid'] ) && is_array( $aisales_settings['sendgrid'] ) ? $aisales_settings['sendgrid'] : array();
$aisales_resend   = isset( $aisales_settings['resend'] ) && is_array( $aisales_settings['resend'] ) ? $aisales_settings['resend'] : array();
$aisales_mailgun  = isset( $aisales_settings['mailgun'] ) && is_array( $aisales_settings['mailgun'] ) ? $aisales_settings['mailgun'] : array();
$aisales_postmark = isset( $aisales_settings['postmark'] ) && is_array( $aisales_settings['postmark'] ) ? $aisales_settings['postmark'] : array();
$aisales_ses      = isset( $aisales_settings['ses'] ) && is_array( $aisales_settings['ses'] ) ? $aisales_settings['ses'] : array();

$aisales_smtp_host       = isset( $aisales_smtp['host'] ) ? $aisales_smtp['host'] : '';
$aisales_smtp_port       = isset( $aisales_smtp['port'] ) ? (int) $aisales_smtp['port'] : 587;
$aisales_smtp_encryption = isset( $aisales_smtp['encryption'] ) ? $aisales_smtp['encryption'] : 'tls';
$aisales_smtp_auth       = ! empty( $aisales_smtp['auth'] );
$aisales_smtp_username   = isset( $aisales_smtp['username'] ) ? $aisales_smtp['username'] : '';
$aisales_smtp_password   = isset( $aisales_smtp['password'] ) ? $aisales_smtp['password'] : '';
$aisales_from_email      = isset( $aisales_smtp['from_email'] ) ? $aisales_smtp['from_email'] : '';
$aisales_from_name       = isset( $aisales_smtp['from_name'] ) ? $aisales_smtp['from_name'] : '';

$aisales_sendgrid_api_key = isset( $aisales_sendgrid['api_key'] ) ? $aisales_sendgrid['api_key'] : '';
$aisales_sendgrid_from_email = isset( $aisales_sendgrid['from_email'] ) ? $aisales_sendgrid['from_email'] : '';
$aisales_sendgrid_from_name = isset( $aisales_sendgrid['from_name'] ) ? $aisales_sendgrid['from_name'] : '';

$aisales_resend_api_key = isset( $aisales_resend['api_key'] ) ? $aisales_resend['api_key'] : '';
$aisales_resend_domain = isset( $aisales_resend['domain'] ) ? $aisales_resend['domain'] : '';
$aisales_resend_from_email = isset( $aisales_resend['from_email'] ) ? $aisales_resend['from_email'] : '';
$aisales_resend_from_name = isset( $aisales_resend['from_name'] ) ? $aisales_resend['from_name'] : '';

$aisales_mailgun_api_key = isset( $aisales_mailgun['api_key'] ) ? $aisales_mailgun['api_key'] : '';
$aisales_mailgun_domain = isset( $aisales_mailgun['domain'] ) ? $aisales_mailgun['domain'] : '';
$aisales_mailgun_region = isset( $aisales_mailgun['region'] ) ? $aisales_mailgun['region'] : 'us';
$aisales_mailgun_from_email = isset( $aisales_mailgun['from_email'] ) ? $aisales_mailgun['from_email'] : '';
$aisales_mailgun_from_name = isset( $aisales_mailgun['from_name'] ) ? $aisales_mailgun['from_name'] : '';

$aisales_postmark_server_token = isset( $aisales_postmark['server_token'] ) ? $aisales_postmark['server_token'] : '';
$aisales_postmark_from_email = isset( $aisales_postmark['from_email'] ) ? $aisales_postmark['from_email'] : '';
$aisales_postmark_from_name = isset( $aisales_postmark['from_name'] ) ? $aisales_postmark['from_name'] : '';

$aisales_ses_access_key = isset( $aisales_ses['access_key'] ) ? $aisales_ses['access_key'] : '';
$aisales_ses_secret_key = isset( $aisales_ses['secret_key'] ) ? $aisales_ses['secret_key'] : '';
$aisales_ses_region = isset( $aisales_ses['region'] ) ? $aisales_ses['region'] : '';
$aisales_ses_from_email = isset( $aisales_ses['from_email'] ) ? $aisales_ses['from_email'] : '';
$aisales_ses_from_name = isset( $aisales_ses['from_name'] ) ? $aisales_ses['from_name'] : '';
?>

<div class="wrap aisales-admin-wrap aisales-mail-provider-page">
	<h1 class="aisales-notices-anchor"></h1>

	<header class="aisales-mail-provider-page__header">
		<div class="aisales-mail-provider-page__header-left">
			<span class="aisales-mail-provider-page__title">
				<span class="dashicons dashicons-email"></span>
				<?php esc_html_e( 'Email Settings', 'stacksuite-sales-manager-for-woocommerce' ); ?>
			</span>
			<span class="aisales-mail-provider-page__subtitle">
				<?php esc_html_e( 'Choose how WooCommerce emails are delivered.', 'stacksuite-sales-manager-for-woocommerce' ); ?>
			</span>
		</div>
		<div class="aisales-mail-provider-page__header-right">
			<?php include AISALES_PLUGIN_DIR . 'templates/partials/balance-indicator.php'; ?>
		</div>
	</header>

	<div class="aisales-mail-provider-page__content">
